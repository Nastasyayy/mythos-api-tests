...
test(
  ...
  { tag: '@ui-example' },
  async ({ page }) => {
    const credentials = await test.step('Create unique credentials for the UI scenario', async () =>
      createUniqueCredentials(),
    );
    ...
    try {
      const registerResponse = await test.step('Register the user through the REST API helper', async () =>
        registerUser(apiRequest, credentials),
      );

      await expect(registerResponse).toBeOK();

      const registerBody = (await registerResponse.json()) as RegisterResponseBody;
